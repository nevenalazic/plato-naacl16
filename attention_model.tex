...
Given this optimization view of $\amax(z_1,\ldots,z_n)$ it is natural to smooth it \cite{nesterov2005smooth} by adding a non-linearity to the optimization. Since the variables are non-negative, one possible choice is an entropy regularizer.\todo{Note that this is not exactly entropy since $\mu$ doesn't normalize to $1$. Explain.} We shall see that this choice results in a closed form solution, and also recovers the standard soft-max case for $K=1$.  Consider the optimization problem:
\be
 \max_{ 
\begin{array}{l}
x_1,\ldots,x_n: \\
...
\be
\samax(z_1,\ldots,z_n) = \sum_{j=1}^{K-1} z_{i_j} + \beta^{-1} \log\sum_{j=K}^n \exp{\beta z_{i_j}}  
\ee
The function $\samax(z_1,\ldots,z_n)$ is differentiable. Denote its gradient with respect to $z_i$ by $v_i$. Also, define:
\bea
c &=& \beta^{-1} \log\sum_{j=K}^n \exp{\beta z_{i_j}} \\
d &=& (K-1) \exp{\beta c} + \sum_{j=K}^n \exp{\beta z_{i_j}}
\eea
...
\left\{
\begin{array}{ll}
\frac{\exp{\beta c}}{d} & j = 1,\ldots,K-1 \\
\frac{\exp{\beta z_{i_j}}}{d} & j = K,\ldots,n 
\end{array} 
\right\}
\ee
\end{proposition}  
