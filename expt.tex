...
\begin{tabular}{l|l|l}
 ...
 \hline
{\bf Caroline} has dropped her name & base: Caroline (/m/01cvzm) &  Democratic Party\\ 
from consideration for the seat   & attn: Caroline Kennedy (/m/01y603) & New York \\
that Hillary has left vacant. & & Robert Kennedy \\ \hline
{\bf Chris Johnson} had just 13 tackles last & base: Chris Johnson (/m/03d753s) & Oakland Raiders  \\
season, and the Raiders currently have & attn: Chris Johnson (/m/027nr9m) & Oakland Raiders \\
...
and the mentions in the document that are attended to (here $K=3$). In brackets
is the Freebase MID. In the first example, the baseline labels the mention ``Caroline'' as the given name, whereas
the attention model attends to mentions that identify it as the diplomat Caroline Kennedy. In the second
example, both models label ``Chris Johnson'' as a football player, but the attention model finds the right one, by attending
to three mentions of Oakland Raiders where /m/027nr9m played.}}}
\end{table*}
