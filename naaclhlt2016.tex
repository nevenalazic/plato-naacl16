...
\begin{abstract}
Entity resolution (ER) is the task of linking mentions of entities in text to their corresponding records in a knowledge base. Coherence models for ER jointly disambiguate all mentions in the document, and encourage a solution in which entities are related to each other. 
In this work, we propose a novel coherence model with multi-focal attention. Rather than aggregating information for an entity over the entire document (which may dilute evidence for entities with few relations), we select a small set of relevant relations for each candidate. 
Our approach enjoys tractable inference and relies on a novel differentiable multi-focus attention function which generalizes softmax. 
